#import "LSTextHeaderCell.h"
#import <Preferences/PSSpecifier.h>
#import "../TintColors.h"

static NSString *const kLSTextPrefsSuite = @"com.strayfade.lstext~prefs";

@implementation LSTextHeaderCell {
	UISwitch *_toggle;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)identifier specifier:(PSSpecifier *)specifier {
	...
	versionLabel.text = @"1.1 (Rootless)";
	versionLabel.font = [UIFont boldSystemFontOfSize:19.0];
	versionLabel.textColor = UIColor.secondaryLabelColor;
	versionLabel.textAlignment = NSTextAlignmentCenter;
	[self.contentView addSubview:versionLabel];

	NSUserDefaults *defaults = [[NSUserDefaults alloc] initWithSuiteName:kLSTextPrefsSuite];
