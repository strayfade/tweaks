#import "Settings26DeveloperCell.h"
#import <Preferences/PSSpecifier.h>

@interface Settings26DeveloperView : UIControl
...
@implementation Settings26DeveloperView {
...
- (void)didMoveToWindow {
	...
	if (self.imageName.length > 0) {
		avatar = [UIImage imageNamed:self.imageName inBundle:[NSBundle bundleForClass:self.class] compatibleWithTraitCollection:nil];
	}
	if (!avatar) {
		...
		avatar = [UIImage systemImageNamed:@"person.crop.circle.fill" withConfiguration:config];
	}
...
@implementation Settings26DeveloperCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)identifier specifier:(PSSpecifier *)specifier {
	...
	Settings26DeveloperView *developerView = [Settings26DeveloperView new];
	...
	developerView.imageName = [specifier propertyForKey:@"developerImage"] ?: @"";
